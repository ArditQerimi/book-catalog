...
const BookCard: React.FC<BookCardProps> = ({ book, onClick }) => {
  return (
    <div 
      ...
      className="group cursor-pointer bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 border border-emerald-50/50 flex flex-col"
    >
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="mt-auto pt-4 flex items-end justify-between border-t border-emerald-50">
          ...
          <div className="flex flex-col items-end">
            <span className="text-lg font-bold text-emerald-900 italic">£{book.price.toFixed(2)}</span>
          </div>
        </div>
      </div>
    </div>

...
const BookDetailClient: React.FC<BookDetailClientProps> = ({ book, allBooks }) => {
  ...
  if (!book) {
    return (
      <div className="py-40 text-center">
         ...
         <button onClick={() => router.push('/')} className="px-8 py-3 bg-emerald-800 text-white font-bold rounded-2xl hover:bg-emerald-900 transition-all">
           Return to Archive
         </button>
      </div>
    );
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-1000">
      <div className="mb-12">
        <button onClick={() => router.back()} className="group flex items-center gap-3 text-[10px] font-bold uppercase tracking-[0.2em] text-emerald-800/40 hover:text-emerald-800 transition-all">
        ...
        <div className="flex-1 p-8 md:p-16 lg:p-20 flex flex-col">
          ...
          <div className="space-y-12 flex-1">
            ...
            <div className="mt-auto pt-12 border-t border-emerald-50">
              <BookStats year={book.year} pages={book.pages} language={book.language} price={book.price || 0} />
              <div className="flex justify-end mt-8">
                 <div className="bg-emerald-50/50 px-6 py-3 rounded-2xl border border-emerald-100 shadow-sm">
                   <span className="text-3xl font-bold text-emerald-950 italic">£{(book.price || 0).toFixed(2)}</span>
                 </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {recommendations.map(rec => (
              <BookCard 
                key={rec.id} 
                book={rec} 
                onClick={(b) => router.push(`/book/${b.id}`)} 
              />
            ))}
          </div>
        </div>
